...
mdFeatures <- topfeatures(dfm(data_char_mobydick, remove=stopwords("en"), remove_punct = T, remove_symbols = T), 100)

# Create a data.frame for ggplot
topDf <- data.frame(
  list(
    term = names(mdFeatures),
    frequency = unname(mdFeatures)
  )
)
...
ggplot(topDf) + geom_line(aes(x=term, y=frequency, group=1), color="blue") +
  theme(axis.text.x=element_text(angle=90, hjust=1))
