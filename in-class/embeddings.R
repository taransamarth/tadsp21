...
wiki_corp<-readRDS(file = "in-class/wiki.RDS")
...
word_vectors["france", , drop = FALSE] + word_vectors["germany", , drop = FALSE]
...
harvard<-  word_vectors["cambridge", , drop = FALSE] - 
  word_vectors["uk", , drop = FALSE] + word_vectors["massachusetts", , drop = FALSE]
cos_sim <- textstat_simil(as.dfm(word_vectors), y = as.dfm(harvard),
                          margin = "documents", method = "cosine")
head(sort(cos_sim[, 1], decreasing = TRUE), 5)
